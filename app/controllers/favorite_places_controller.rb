class FavoritePlacesController < ApplicationController
  ...
  def index
    ...
    if !@user.nil?
      @places = @user.my_favorite_places.where("latitude > ? AND latitude < ? AND longitude > ? AND longitude < ?", params[:b1], params[:b3], params[:b2], params[:b4]).includes(:default_place_photo, :category, :city => [:region, :country]).order("updated_at DESC").page(params[:page]).per(50)
